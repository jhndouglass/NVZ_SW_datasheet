...
This document contains a summary of the evidence used to designate NVZ `r nvz.id` - `r nvz.name.short`. It was produced `r date` by the Environment Agency on behalf of the Secretary of State for the Environment.
...
if (sep.yes) {
    tab.count <- TabCount(tab.count)
    fig.count <- FigCount(fig.count)
    sep.labels.text <- paste0("Table ",  tab.count, " shows the load SEPERATE estimates is generated persector. Figure ", fig.count, " shows the same data expressed as a percentage of the total load. Note that for main river designations data is provided for the entire upstream area, while for tributary designations only data from the designating waterbody is provided.")
    cat("\n", sep.labels.text, "\n")
} else {cat("\n", sep.pholder.text, "\n")}
```
