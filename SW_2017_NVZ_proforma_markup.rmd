...
if (sep.yes) {
    tab.count <- TabCount(tab.count)
    fig.count <- FigCount(fig.count)
    sep.labels.text <- paste0("Table ",  tab.count, " shows the load SEPARATE estimates is generated persector. Figure ", fig.count, " shows the same data expressed as a percentage of the total load. Note that for main river designations data is provided for the entire upstream area, while for tributary designations only data from the designating waterbody is provided.")
    cat("\n", sep.labels.text, "\n")
} else {cat("\n", sep.pholder.text, "\n")}
```
