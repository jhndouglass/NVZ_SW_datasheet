...
`r nvz.name.short` is `r nvz.type1.text` designation. `r nvz.mod.text` There `r fail.count.text` in the waterbody that designates this NVZ, and, in total, `r fail.count.text.upstream` in the entire NVZ including the designating waterbody and any area upstream of the designating waterbody. `r mon.change.text.long` `r mod.change.text.long` `r nvz.des.text`  
...
Please note that the area of NVZ designation may change during the appeals process. The definitive NVZ area can be seen on the What's in Your Backyard (WIYBY) website [http://apps.environment-agency.gov.uk/wiyby/141443.aspx](apps.environment-agency.gov.uk/wiyby/141443.aspx).
...
NVZ type (existing/modified/new)             `r nvz.type1`
...
`r nvz.name.short` is `r nvz.type1.text` designation. `r nvz.mod.text` `r mon.change.text.long` `r mod.change.text.long` `r nvz.des.text`
...
mon.yes <- FALSE
mon.pholder.text <- paste0("Monitoring evidence was not used in this designation. There were not any monitoing points with sufficent data in the designation waterbody. Table ", tab.count, " will contain NA (Not Available) values")
...
if (load.yes) {
        tab.count <- TabCount(tab.count)
    load.text1 <- paste0("We present two figures for load from consented discharges. The maximum value is based on discharge consent conditions (the conditions consented discharges may not exceed). Therefore the maximum value represents the load emitted from a combination of the greatest flow and the greatest concentration of nitrogen compounds all consented discharges in the catchment may legally emit. The minimum value is based on information from consented discharges with flow and nitrogen concentration monitoring, so represents the most likely load from all consented discharges in the catchment.
...
} else{cat("There are no samples points with sufficient monitoring data within this NVZ")}
```
